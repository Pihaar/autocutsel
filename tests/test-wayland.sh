...
assert_contains "CLIPBOARD on Wayland → target PRIMARY" "$_output" "Target selection: PRIMARY"

# PRIMARY monitored on Wayland → target = CLIPBOARD
run_capture_unbuffered 2 "$AUTOCUTSEL" -selection PRIMARY -debug
assert_contains "PRIMARY on Wayland → target CLIPBOARD" "$_output" "Target selection: CLIPBOARD"
...
assert_not_contains "no target selection in X11 mode" "$_output" "Target selection:"
